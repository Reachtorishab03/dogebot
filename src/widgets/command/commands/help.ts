...
const help = {
  fun: {
    ...
    bowcards:
      "Who doesn't love to play with cards! Try it, feel free to make up some cool game of cards using it",
    bowmeme: "Who doesn't love memes! Try it out.",
    isthisforthat:
      "Need some quirky ideas for startup? Type it down fast then <:doge:824583822502133780>",
  ...
  info: {
    avatar:
      "Get a bigger image of a user's avatar, don't embarass them though.",
    help: "Provides you with the command list",
    userinfo: "Shows information about you or the specified user",
    ping:
      "Show's Doge's latency to Discord's API, hard words eh? I dont understand either.",
  },
  utility: {
...
module.exports = new CommandBuilder()
  ...
  .setExecute(async (message, user, args) => {
    if (args.length == 0) {
      ...
      embed.addField(
        ":musical_note: Music",
        `\`${Object.keys(help.music).join("`, `")}\``
      );
      embed.addField(
        ":information_source: Info",
        `\`${Object.keys(help.info).join("`, `")}\``
      );
      embed.addField(
        ":wrench: Utility",
        `\`${Object.keys(help.utility).join("`, `")}\``
      );
      embed.setColor("#d6a844");
      embed.setFooter(
        "To get more info on a command, do .help category.command eg:`.help fun.bowtalk`"
      );
